...
export function useSettings(
  adapter: PersistenceAdapter<AppSettings> = defaultAdapter,
) {
  ...
  const setBackupReminderEnabled = useCallback(
    (enabled: boolean) => {
  ...
  const setLastBackupPrompt = useCallback(
    (date: string) => {
  ...
  const setShuffleAnswers = useCallback(
    (enabled: boolean) => {
  ...
  return {
    ...
    setGameMode,
    backupReminderEnabled: settings.backupReminderEnabled ?? true,
