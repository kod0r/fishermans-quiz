      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-950 via-slate-900 to-teal-950">
        <div className="container mx-auto px-4 py-8 max-w-4xl pt-16">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-white">Quiz-Fortschritt</h1>
            <p className="text-slate-400 mt-2">Aktueller Durchlauf</p>
          </div>

          {/* Run-Ergebnis */}
          <Card className={`mb-6 ${passed ? 'bg-emerald-900/20 border-emerald-500/30' : 'bg-red-900/20 border-red-500/30'}`}>
            <CardContent className="pt-8 pb-8 text-center">
              <div className={`w-24 h-24 rounded-full mx-auto mb-4 flex items-center justify-center ${passed ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
                {passed ? <CheckCircle className="w-12 h-12 text-emerald-400" /> : <XCircle className="w-12 h-12 text-red-400" />}
              </div>
              <h2 className={`text-2xl font-bold mb-2 ${passed ? 'text-emerald-400' : 'text-red-400'}`}>{passed ? 'Bestanden!' : 'Nicht bestanden'}</h2>
              <p className="text-slate-300 mb-6">{statistiken.korrekt} von {statistiken.gesamt} richtig</p>
              <div className="max-w-md mx-auto">
                <Progress value={pct} className="h-4 bg-slate-700" />
                ...
                {Object.entries(bereichStats).map(([b, s]) => {
                  ...
                  return (
                    <div key={b}>
                      ...
                      <Progress value={p} className="h-2 bg-slate-700" />
                    </div>
                  );
                })}
              </div>
          ...
          {falsche.length > 0 && (
            <Card className="mb-6 bg-slate-800/50 border-slate-700/50">
              <CardHeader>
                <button onClick={() => setShowWrong(!showWrong)} className="w-full flex items-center justify-between">
                  <CardTitle className="text-white flex items-center gap-2"><XCircle className="w-5 h-5 text-red-400" />Falsche Antworten ({falsche.length})</CardTitle>
                  <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform ${showWrong ? 'rotate-180' : ''}`} />
                </button>
              </CardHeader>
              {showWrong && (
              <CardContent>
                <div className="space-y-4">
                  {falsche.map(frage => {
                    ...
                    return (
                      <div key={frage.id} className="p-4 rounded-xl bg-red-500/5 border border-red-500/10">
                        <div className="flex items-start justify-between gap-4 mb-2">
                          <p className="text-white font-medium text-sm">{frage.frage}</p>
                          <button onClick={() => { const idx = aktiveFragen.findIndex(f => f.id === frage.id); if (idx >= 0) springeZuFrage(idx); }} className="text-teal-400 text-xs hover:underline whitespace-nowrap">Zur Frage</button>
                        </div>
          ...
          {offen.length > 0 && (
            <Card className="mb-6 bg-slate-800/50 border-slate-700/50">
              <CardHeader>
                <button onClick={() => setShowUnanswered(!showUnanswered)} className="w-full flex items-center justify-between">
                  <CardTitle className="text-white flex items-center gap-2"><HelpCircle className="w-5 h-5 text-amber-400" />Unbeantwortete Fragen ({offen.length})</CardTitle>
                  <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform ${showUnanswered ? 'rotate-180' : ''}`} />
                </button>
              </CardHeader>
              {showUnanswered && (
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {offen.map(frage => {
                      ...
                      return <button key={frage.id} onClick={() => springeZuFrage(idx)} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20 hover:bg-amber-500/20 transition-all">Frage {idx + 1}</button>;
                    })}
                  </div>
                </CardContent>
              )}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Tooltip>
              <TooltipTrigger asChild>
                <Button onClick={onBackToStart} variant="outline" className="border-slate-600 text-slate-300 hover:bg-slate-800 px-8">Zurück zur Startseite</Button>
              </TooltipTrigger>
              <TooltipContent><p>Quiz läuft weiter</p></TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button onClick={onBackToQuiz} className="bg-teal-500 hover:bg-teal-600 text-white px-8">Zurück zum Quiz</Button>
              </TooltipTrigger>
