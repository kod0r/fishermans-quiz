    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper has-[[data-variant=inset]]:bg-sidebar flex [min-height:100svh] w-full",
            className
          )}
          {...props}
        >
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "fixed inset-y-0 z-10 hidden [height:100svh] w-[var(--sidebar-width)] transition-[left,right,width] duration-200 ease-linear md:flex",
          side === "left"
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(() => {
    const hash = className?.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0) ?? 0;
    return `${(hash % 40) + 50}%`
  }, [className])
